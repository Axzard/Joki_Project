...
class SyalPage extends StatelessWidget {
  ...
  // URL untuk menyimpan pesanan ke Firebase
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        data.forEach((key, productData) {
          if (key == 'Syal') { // Menyaring data untuk kategori 'Syal'
  ...
  Future<void> saveToCart(Product product, String size) async {
    try {
      ...
      final response = await http.post(
        ...
        body: jsonEncode(orderData),  // Kirim data pesanan
      );
  ...
  Widget build(BuildContext context) {
    ...
    String? selectedSize; // Menyimpan ukuran yang dipilih
